...
def build_initialiser(
    ...
    if board.arch.is_x86():
        rust_sel4_dir = Path(__file__).resolve().parent.parent / "rust-sel4-offline"
        rust_target_dir = rust_sel4_dir / "support" / "targets"
        toolchains = subprocess.check_output(["rustup", "toolchain", "list"], text=True).splitlines()
        nightly = next((line.split()[0] for line in toolchains if line.startswith("nightly")), None)
        if nightly is None:
            raise Exception("Could not find a nightly Rust toolchain required for the x86-64 initializer build")

        env = environ.copy()
        env["SEL4_PREFIX"] = str(sel4_src_dir.resolve())
        env["RUST_TARGET_PATH"] = str(rust_target_dir.resolve())
        cargo_cmd = [
            "cargo",
            f"+{nightly}",
            "build",
            ...
            "--target",
            "x86_64-sel4-roottask-minimal",
            "--release",
            "--manifest-path",
            "crates/sel4-capdl-initializer/Cargo.toml",
        ]
        r = subprocess.run(cargo_cmd, cwd=rust_sel4_dir, env=env)
        if r.returncode != 0:
            raise Exception(
                f"Error building: {component_name} for board: {board.name} config: {config.name}"
            )
        capdl_init_elf = rust_sel4_dir / "target" / "x86_64-sel4-roottask-minimal" / "release" / "sel4-capdl-initializer.elf"
    else:
        cargo_target = board.arch.rust_toolchain()
        initialiser_src = Path(__file__).resolve().parent / "initialiser-offline" / "initialiser.c"
        initialiser_out_dir = build_dir / component_name / cargo_target / "release"
        initialiser_out_dir.mkdir(exist_ok=True, parents=True)
        gcc = shutil.which("riscv64-elf-gcc") or shutil.which("riscv64-unknown-elf-gcc")
        if gcc is None:
            raise Exception(
                "Could not find a RISC-V bare-metal C compiler (tried riscv64-elf-gcc and riscv64-unknown-elf-gcc)"
            )
        gcc_flags = "-march=rv64gc -mabi=lp64"
        capdl_init_elf = initialiser_out_dir / "initialiser"
        r = system(
            f"{gcc} -Os -ffreestanding -fno-builtin -fno-stack-protector "
            f"-nostdlib -nostartfiles -static -fno-pie -no-pie -Wl,-e,_start "
            f"{gcc_flags} -I{sel4_src_dir}/libsel4/include -o {capdl_init_elf} {initialiser_src}"
        )
        if r != 0:
            raise Exception(
                f"Error building: {component_name} for board: {board.name} config: {config.name}"
